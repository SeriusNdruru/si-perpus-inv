...
<div class="table-wrap"><table><thead><tr><th class="table-number-heading">No.</th><th>Kode</th><th>Nama</th><th>Induk</th><th>Cakupan</th><th class="table-actions-heading">Aksi</th></tr></thead><tbody>
@forelse ($categories as $record)
<tr><td class="table-number">{{ (is_object($categories) && method_exists($categories, 'firstItem') && $categories->firstItem() !== null ? $categories->firstItem() : 1) + $loop->index }}</td><td><strong>{{ $record->category_code }}</strong></td><td>{{ $record->category_name }}</td><td>{{ $record->parent?->category_name ?? 'Kategori utama' }}</td><td>{{ match ($record->scope) { 'inventory' => 'Inventaris', 'library' => 'Perpustakaan', default => 'Bersama' } }}</td><td><form method="POST" action="{{ route('categories.deleted.restore', $record) }}">@csrf @method('PATCH')<button class="action-button">Pulihkan</button></form></td></tr>
