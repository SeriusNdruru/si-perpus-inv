...
<div class="table-wrap"><table><thead><tr><th class="table-number-heading">No.</th><th>Kode</th><th>Nama</th><th>Deskripsi</th><th>Jumlah barang</th><th class="table-actions-heading">Aksi</th></tr></thead><tbody>
@forelse ($units as $record)
<tr><td class="table-number">{{ (is_object($units) && method_exists($units, 'firstItem') && $units->firstItem() !== null ? $units->firstItem() : 1) + $loop->index }}</td><td><strong>{{ $record->unit_code }}</strong></td><td>{{ $record->unit_name }}</td><td>{{ $record->description ?: '-' }}</td><td>{{ number_format((int) $record->items_count) }}</td><td><form method="POST" action="{{ route('units.deleted.restore', $record) }}">@csrf @method('PATCH')<button class="action-button">Pulihkan</button></form></td></tr>
