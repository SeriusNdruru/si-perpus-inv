...
<div class="table-wrap"><table><thead><tr><th>Kode</th><th>Supplier</th><th>Kontak</th><th>Jumlah aset</th><th class="table-actions-heading">Aksi</th></tr></thead><tbody>
@forelse ($suppliers as $record)
<tr><td><strong>{{ $record->supplier_code }}</strong></td><td>{{ $record->supplier_name }}</td><td>{{ $record->contact_person ?: '-' }}</td><td>{{ number_format((int) $record->assets_count) }}</td><td><form method="POST" action="{{ route('suppliers.deleted.restore', $record) }}">@csrf @method('PATCH')<button class="action-button">Pulihkan</button></form></td></tr>
