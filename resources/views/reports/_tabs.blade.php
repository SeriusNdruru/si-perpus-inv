...
<nav class="report-tabs" aria-label="Jenis laporan">
    ...
    @if ($canLibraryReports)
        ...
        <a href="{{ route('reports.loans') }}" class="{{ request()->routeIs('reports.loans*', 'reports.loan-records*') ? 'active' : '' }}">Riwayat Peminjaman</a>
        <a href="{{ route('reports.library-visits') }}" class="{{ request()->routeIs('reports.library-visits*', 'reports.frequent-visitors*') ? 'active' : '' }}">Riwayat Kunjungan</a>
