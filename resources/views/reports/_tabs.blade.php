...
<nav class="report-tabs" aria-label="Jenis laporan">
    ...
    @if ($canLibraryReports)
        ...
        <a href="{{ route('reports.loans') }}" class="{{ request()->routeIs('reports.loans*') ? 'active' : '' }}">Riwayat Peminjaman</a>
        <a href="{{ route('reports.loan-records') }}" class="{{ request()->routeIs('reports.loan-records*') ? 'active' : '' }}">Peringkat Peminjaman</a>
