...
<div class="table-wrap"><table><thead><tr><th class="table-number-heading">No.</th><th>Kode</th><th>Rak</th><th>Lokasi</th><th>Klasifikasi</th><th class="table-actions-heading">Aksi</th></tr></thead><tbody>
@forelse ($shelves as $record)
<tr><td class="table-number">{{ (is_object($shelves) && method_exists($shelves, 'firstItem') && $shelves->firstItem() !== null ? $shelves->firstItem() : 1) + $loop->index }}</td><td><strong>{{ $record->shelf_code }}</strong></td><td>{{ $record->shelf_name }}</td><td>{{ $record->location?->location_name ?? '-' }}</td><td>{{ $record->classification_range ?: '-' }}</td><td><form method="POST" action="{{ route('library.shelves.deleted.restore', $record) }}">@csrf @method('PATCH')<button class="action-button">Pulihkan</button></form></td></tr>
