...
<section class="panel">
    ...
    <div class="table-wrap"><table><thead><tr><th class="table-number-heading">No.</th><th>Pengirim</th><th>Subjek</th><th>Status</th><th>Tanggal</th><th>Aksi</th></tr></thead><tbody>
        @forelse ($messages as $message)
            <tr><td class="table-number">{{ (is_object($messages) && method_exists($messages, 'firstItem') && $messages->firstItem() !== null ? $messages->firstItem() : 1) + $loop->index }}</td><td><strong>{{ $message->sender_name }}</strong><div class="table-secondary">{{ $message->sender_email ?: ($message->sender_phone ?: '-') }}</div></td><td>{{ $message->subject }}</td><td><span class="badge {{ $message->status === 'unread' ? 'badge-warning' : 'badge-neutral' }}">{{ ucfirst($message->status) }}</span></td><td>{{ $message->created_at?->format('d/m/Y H:i') }}</td><td><a class="action-link" href="{{ route('library.contact-messages.show', $message) }}">Buka</a></td></tr>
        @empty<tr><td colspan="6" class="empty-state">Belum ada pesan.</td></tr>@endforelse
    </tbody></table></div>
</section>
@endsection
